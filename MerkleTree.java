...
public class MerkleTree {
  ...
  public boolean siblingCoupledList(List<Pair<String,String>> path, Transaction transaction, TreeNode root){
        if (root == null) {
            return false;
        }
        if (get_str(transaction) == root.val){
          return true;
        }
        if (siblingCoupledList(path, transaction, root.left)) {
            path.add(new Pair<>(root.left.val, root.right.val));
            return true;
        }
        if (siblingCoupledList(path, transaction, root.right)) {
            path.add(new Pair<>(root.right.val, root.left.val));
            return true;
        }
        return false; 
  }
